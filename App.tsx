...
function MainTabs({ onLoggedOut }: { onLoggedOut: () => void }) {
  ...
  return (
    <Tab.Navigator
      ...
      <Tab.Screen name="Cases" component={CaseTrackerScreen} options={{ tabBarLabel: t('track_title'), tabBarIcon: tabIcon('📂') }} />
      <Tab.Screen name="Voice" component={SpeakToUsScreen} options={{ tabBarLabel: t('dash_speak'), tabBarIcon: tabIcon('🎤') }} />
      <Tab.Screen name="SOS" component={SOSScreen} options={{ tabBarLabel: 'SOS', tabBarIcon: tabIcon('🚨'), tabBarActiveTintColor: colors.error }} />
...
type AuthState = 'loading' | 'login' | 'signup' | 'pending' | 'in';
  ...
  return (
    <SafeAreaProvider>
      ...
      {authState === 'loading' ? (
        ...
        <LoginScreen onLoggedIn={afterLogin} onSignUp={() => setAuthState('signup')} />
      ) : authState === 'signup' ? (
        <SignupScreen
